...
이 저장소의 코드는 원 저자의 코드에서 다음의 내역을 수정한 것입니다.
 - node.js 버전을 12에서 18로 변경하였습니다. 11장에서 사용하는 Dockerfile의 베이스 이미지 역시 node 18 버전을 사용하도록 변경하였습니다.
 - 일부 의존성 패키지의 호환성, 보안 문제 해결을 위해 next.js 패키지 버전을 13.1.1, react와 react-dom 패키지 버전을 18.2.0으로 업그레이드 하였습니다. Next.js 버전을 변경하여도 책의 코드와 차이나는 부분은 없으니, 책의 예시 코드를 그대로 따라하셔도 무방합니다.
 - 계약 문제로 종료된 원격 API / GraphQL 서버를 대체하기 위하여 4장의 API / GraphQL 서버 코드를 추가하였습니다.
